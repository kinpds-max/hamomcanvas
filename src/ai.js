...
export function initAI() {
  ...
  // Initialize with a rich default description welcome guide
  const welcomeText = `👋 **하맘캔버스 AI 도우미에 오신 것을 환영합니다!**

하맘캔버스는 피그마처럼 무한한 작업판(캔버스) 위에 생각과 자료를 자유롭게 정리하는 도구입니다.

💡 **하맘캔버스 핵심 조작법 (기본 설명)**:
1. **더블클릭**: 캔버스 빈 곳을 더블클릭하면 즉시 새 텍스트 카드가 생성됩니다.
2. **연결선(C)**: \`C\` 키를 누르고 카드를 차례대로 클릭해 서로 연결합니다. (\`Delete\` 키로 삭제)
3. **화면 이동**: \`Space\` 키를 누른 채 마우스로 드래그하면 화면이 움직입니다.
4. **섹션 정리**: 대형 섹션 카드 안에 일반 카드를 넣고 한 번에 묶어 그룹으로 이동합니다.
5. **링크 수집**: 주소(URL)만 넣으면 제목과 썸네일을 자동으로 긁어오며, 마우스를 올리면 실시간 미리보기가 노출됩니다.
6. **공유**: 상단 **[공유하기]**를 통해 작업 중인 화면 전체를 카카오톡으로 즉시 친구에게 보낼 수 있습니다.

궁금한 기능이 있다면 아래 추천 질문을 클릭하시거나 직접 편하게 물어보세요! 👇`;
